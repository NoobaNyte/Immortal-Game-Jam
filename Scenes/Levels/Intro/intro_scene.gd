...
@export_file_path var level_to_transition_to_after_intro

@export var click_screen_button: Button
...
@export var pulse_duration_min: float = 8.0 ## Increased from 2.0
@export var pulse_duration_max: float = 14.0 ## Increased from 4.5
@export var wobble_distance: float = 35.0 ## Reduced from 60.0 for less aggressive drifting
...
func _unhandled_input(event: InputEvent) -> void:
	# Trigger next slide on standard UI Accept
	var is_spacebar = event is InputEventKey and event.keycode == KEY_SPACE and event.pressed
	
	if is_spacebar or event.is_action_pressed("ui_accept"):
		_next_slide()
...
func _randomize_camera_pulse() -> void:
	...
	# Exclusively use TRANS_SINE for the most consistent, buttery-smooth acceleration/deceleration
...
func _finish_intro_sequence() -> void:
	...
	# Fire the global signal while the screen is black
	Global.load_next_level.emit(level_to_transition_to_after_intro, 0, 2.0)
...
func _on_click_screen_button_pressed() -> void:
	_next_slide()
